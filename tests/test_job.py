...
class TestKeepalive(object):
    ...
    @pytest.mark.timeout(1.0)
    def test_stop_while_server_down(self, bg_version_connect, s,
                                    no_config_files):
...
def test_get_machine_info(bg_version_connect, s, j, no_config_files,
                          allocated):
    if allocated:
        s.send({"return": {
            "width": 8, "height": 8,
...
class TestWaitForStateChange(object):
    ...
    @pytest.mark.timeout(1.0)
    def test_state_already_changed(self, bg_version_connect, s, j,
                                   no_config_files):
    ...
    @pytest.mark.timeout(1.0)
    def test_impossible_timeout(self, bg_version_connect, s, j,
                                no_config_files):
...
class TestWaitUntilReady(object):
    ...
    def test_bad_state(bg_version_connect, s, j, no_config_files,
        ...
        # state
        j.get_state = Mock(
            return_value=JobStateTuple(state=final_state, power=None,
    ...
    def test_timeout(bg_version_connect, s, j, no_config_files):
        ...
        j.get_state = Mock(
            return_value=JobStateTuple(state=JobState.power,
                                       power=True, keepalive=60.0,
                                       reason=None))
